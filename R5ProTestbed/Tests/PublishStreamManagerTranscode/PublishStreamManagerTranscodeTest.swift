...
class PublishStreamManagerTranscodeTest: BaseTest {
    ...
    func responder(urls: Array<String>, streamName: String) -> (String?, Error?) -> Void {
        var urls = urls
        return {(ip: String?, error: Error?) -> Void in
            ...
            if ((error) != nil) {
                if (urls.endIndex == urls.startIndex) {
                    ...
                    self.requestOrigin(urls.popLast()!, resolve: self.responder(urls: urls, streamName: streamName))
                }
                return;
            }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let originURI = (Testbed.getParameter(param: "host") as! String) + portURI + "/streammanager/api/3.0/event/" +
            (Testbed.getParameter(param: "context") as! String) + "/" +
            (Testbed.getParameter(param: "stream1") as! String) + "?action=broadcast&transcode=true"
        
        let httpString = "http://" + originURI
        let httpsString = "https://" + originURI
        
        let topLevelStreamName = (Testbed.getParameter(param: "stream1") as! String) + "_1"
        
        var urls = [httpString, httpsString]
        
        requestOrigin(urls.popLast()!, resolve: responder(urls: urls, streamName: topLevelStreamName))
    }
