...
//  R5ProTestbed
//
//  Created by David Heimann on 9/30/20.
//  Copyright © 20202 Infrared5. All rights reserved.
//
...
@objc(PublishSocialPushTest)
class PublishSocialPushTest: PublishTest, UITextViewDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        rtmpInput = UITextView(frame: CGRect(x: 0, y: screenSize.height - 24, width: (screenSize.width * 0.6) - 50, height: 24) )
...
//        rtmpInput?.text = String(format: "rtmp://%@:1935/%@/%@", host, context, stream2)
    ...
    @objc func doPush() {
        ...
        var url: String
        if(port == "443"){
            url = String(format: httpsFormat, host, urlParams)
        }
        else{
            url = String(format: httpFormat, host, port, urlParams)
        }
    ...
    func generateParams( action: String) -> String {
        ...
        let prehash = String(format: "%@%ld%@", action, timestamp, pass).data(using: String.Encoding.utf8)
        
        var hash = [UInt8](repeating: 0, count: Int(CC_SHA256_DIGEST_LENGTH))
        prehash!.withUnsafeBytes {
            _ = CC_SHA256($0.baseAddress, CC_LONG(prehash!.count), &hash)
        }
        let signiture = hash.map { String(format: "%x", $0) }.joined() //No leading zeroes
        
        return String(format: "?action=%@&timestamp=%ld&signature=%@", action, timestamp, signiture)
    }
